...
```{r Total Scores}
total <- read.xlsx(file = "../data/scoreList.xlsx", sheetIndex = 1, rowIndex = 3:71, colIndex = 2:10, colClasses = c(rep("character", 2), rep("numeric", 7)), header = TRUE)
...
total.order$Grades <- ifelse(total.order$Ranks <= floor(67*0.3), "A+", ifelse(total.order$Ranks <= floor(67*0.6), "B+", "C+"))
kable(total.order)
```
