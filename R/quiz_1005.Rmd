...
library(ggplot2)
options(digits=3)
quiz1005 <- read.xlsx("../data/class_roll1005.xlsx", 1, startRow = 2, endRow = 72, colIndex = c(3:7, 9:13), colClasses=rep("character", 10))
str(quiz1005)
names(quiz1005) <- c("dept", "id", "name", "year", "e.mail", "cell.no", "Q1", "Q2", "Q3", "Q4")
str(quiz1005)
q.ans <- quiz1005[paste("Q", 1:4, sep="")]
q.ans.pdi <- factor(q.ans$Q1, levels=c("가", "나", "다", "라", "마"), labels=paste(rev(seq(50, 90, by=10)), "%", sep=""))
q.ans.mean.median <- factor(q.ans$Q4, levels=c("가", "나"), labels=c("평균", "중위수"))
```
...
pander(table(q.ans.mean.median))
```
...
```{r}
(marks <- mapply(function(x, y) factor(ifelse(x == y, "R", "W")), y = q.key.l, quiz1005[c("Q1", "Q2", "Q3", "Q4")]))
```
...
```{r}
barplot(marks.tbl.2, col=c("cyan", "red"), names.arg = v.names)
title(main="10월 5일 퀴즈 결과", xlab="문항", ylab="인원")
```
...
library(reshape2)
str(marks)
dimnames(marks) <- list("ID" = quiz1005$id, "Items" = dimnames(marks)[[2]])
...
ls()
(ml.3 <- ml.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("10월 5일 퀴즈 결과"))
```
...
```{r}
(mtl.3 <- mtl.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("10월 5일 퀴즈 결과"))
```
