...
library(ggplot2)
options(digits=3)
quiz1007 <- read.xlsx("../data/class_roll1007.xlsx", 1, startRow = 2, endRow = 72, colIndex = c(3:7, 9:12), colClasses=rep("character", 10))
str(quiz1007)
names(quiz1007) <- c("dept", "id", "name", "year", "e.mail", "cell.no", "Q1", "Q2", "Q3")
str(quiz1007)
q.ans <- quiz1007[paste("Q", 1:3, sep="")]
q.ans.pdi <- factor(q.ans$Q1, levels=c("가", "나", "다", "라", "마"), labels=paste(rev(seq(50, 90, by=10)), "%", sep=""))
q.ans.reading <- factor(q.ans$Q2, levels=c("가", "나", "다", "라"), labels=c("20대", "30대", "40대", "50대 이상"))
q.ans.hard.times <- factor(q.ans$Q3, levels=c("가", "나", "다", "라"), labels=c("봉급생활자", "자영업자", "중소기업", "대기업"))
```

* 응답 내용 분석
     + 국민총소득 대비 가계처분가능소득의 비율에 대한 인식 향상
     + 자영업자의 현실에 대한 인식
...
pander(table(q.ans.reading))
pander(table(q.ans.hard.times))
```
...
```{r}
q.key <- c("마", "라", "나")
(q.key.l <- split(q.key, f=paste("A", 1:3, sep="")))
str(q.key.l)
```
...
```{r}
(marks <- mapply(function(x, y) factor(ifelse(x == y, "R", "W")), y = q.key.l, quiz1007[c("Q1", "Q2", "Q3")]))
```
...
```{r}
barplot(marks.tbl.2, col=c("cyan", "red"), names.arg = v.names)
title(main="10월 7일 퀴즈 결과", xlab="문항", ylab="인원")
```
...
library(reshape2)
str(marks)
dimnames(marks) <- list("ID" = quiz1007$id, "Items" = dimnames(marks)[[2]])
...
ls()
(ml.3 <- ml.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("10월 7일 퀴즈 결과"))
```
...
```{r}
(ml.8 <- ml.7 + annotate("text", x=1:3, y=rep(10, 3), label=paste(correct.ans.rate, "%", sep="")))
```
...
```{r}
(mtl.3 <- mtl.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("10월 7일 퀴즈 결과"))
```
...
```{r}
(mtl.8 <- mtl.7 + annotate("text", x=1:3, y=rep(10, 3), label=paste(correct.ans.rate, "%", sep="")))
```

## 자료 갈무리

```{r}
save.image("quiz_1007.rda")
```
