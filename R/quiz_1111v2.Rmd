...
library(ggplot2)
options(digits=3)
quiz1111 <- read.xlsx("../data/class_roll1111.xlsx", 1, startRow = 2, endRow = 72, colIndex = c(3:7, 9:13), colClasses=rep("character", 10))
str(quiz1111)
names(quiz1111) <- c("dept", "id", "name", "year", "e.mail", "cell.no", "Q1", "Q2", "Q3", "Q4")
str(quiz1111)
q.ans <- quiz1111[paste("Q", 1:4, sep="")]
q.ans.lm3 <- factor(q.ans$Q1, levels=c("가", "나", "다", "라"), labels=c("12억", "20억", "25억", "30억"))
q.ans.agri.major <- factor(q.ans$Q2, levels=c("가", "나", "다", "라"), labels=c("20%", "40%", "60%", "80%"))
q.ans.food.self.suff <- factor(q.ans$Q3, levels=c("가", "나", "다", "라"), labels=c("미국:125%", "영국:101%", "프랑스:174%", "캐나다:40%"))
q.ans.food.ss.kr <- factor(q.ans$Q4, levels=c("가", "나", "다", "라"), labels=c("20%대", "30%대", "40%대", "50%대"))
```

* 응답 내용 분석
     + 지역승수에 대한 인식 개선(정답률 : 57% => 60%)
     + 5대 곡물메이저의 시장점유율 인식 우수(정답률 89%) 
     + 서방선진국 곡물자급율 인식 우수(정답률 : 85%)
     + 우리나라 전체곡물자급율 인식 우수(정답률 : 85%)

```{r, cross-tables}
load("../R/quiz1028v2.rda")
quiz1111.div <- data.frame(quiz1028.div[, 1:7], quiz1111[c("Q1", "Q2", "Q3", "Q4")])
pander(table(quiz1111.div$college, quiz1111.div$Q1))
pander(table(quiz1111.div$college, quiz1111.div$Q2))
pander(table(quiz1111.div$college, quiz1111.div$Q3))
pander(table(quiz1111.div$college, quiz1111.div$Q4))
```

```{r, mapply and xtabs}
pander(mapply(function(x) xtabs(~college + x, data=quiz1111.div), quiz1111.div[c("Q1", "Q2", "Q3", "Q4")], SIMPLIFY = FALSE))
```

```{r, mosaicplots, fig.width = 6, fig.height = 6}
mosaicplot(table("단과대학"=quiz1111.div$college, "Q1"=quiz1111.div$Q1), main="단과대학별 응답(Q1)")
mosaicplot(table("단과대학"=quiz1111.div$college, "Q2"=quiz1111.div$Q2), main="단과대학별 응답(Q2)")
mosaicplot(table("단과대학"=quiz1111.div$college, "Q3"=quiz1111.div$Q3), main="단과대학별 응답(Q3)")
mosaicplot(table("단과대학"=quiz1111.div$college, "Q4"=quiz1111.div$Q4), main="단과대학별 응답(Q4)")
```

```{r, mapply with mosaicplot, fig.width = 6, fig.height = 6}
mapply(function(x) mosaicplot(xtabs(~college + x, data=quiz1111.div), main=paste("단과대학별 응답(", dimnames(x)[[2]],")", sep=""), xlab = "단과대학", ylab = names(x)), quiz1111.div[c("Q1", "Q2", "Q3", "Q4")])
```
...
```{r}
(marks <- mapply(function(x, y) factor(ifelse(x == y, "R", "W")), y = q.key.l, quiz1111[c("Q1", "Q2", "Q3", "Q4")]))
```
...
```{r}
barplot(marks.tbl.2, col=c("cyan", "red"), names.arg = v.names)
title(main="11월 11일 퀴즈 결과", xlab="문항", ylab="인원")
```
...
library(reshape2)
str(marks)
dimnames(marks) <- list("ID" = quiz1111$id, "Items" = dimnames(marks)[[2]])
...
ls()
(ml.3 <- ml.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("11월 11일 퀴즈 결과"))
```
...
```{r}
(mtl.3 <- mtl.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("11월 11일 퀴즈 결과"))
```
