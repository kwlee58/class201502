...
library(ggplot2)
options(digits=3)
quiz1125 <- read.xlsx("../data/class_roll1125.xlsx", 1, startRow = 2, endRow = 72, colIndex = c(3:7, 9:13), colClasses=rep("character", 10))
str(quiz1125)
names(quiz1125) <- c("dept", "id", "name", "year", "e.mail", "cell.no", "Q1", "Q2", "Q3", "Q4")
str(quiz1125)
q.ans <- quiz1125[paste("Q", 1:4, sep="")]
q.ans.lm3 <- factor(q.ans$Q1, levels=c("가", "나", "다", "라"), labels=c("12억", "20억", "25억", "30억"))
q.ans.monty.hall <- factor(q.ans$Q2, levels=c("가", "나", "다", "라"), labels=c("10%", "30%", "50%", "70%"))
q.ans.wason1 <- factor(q.ans$Q3, levels=c("가", "나", "다", "라"), labels=c("10%이내", "30%", "50%", "70%"))
q.ans.wason2 <- factor(q.ans$Q4, levels=c("가", "나", "다", "라"), labels=c("10%이내", "20%", "30%", "50%이상"))
```

* 응답 내용 분석
     + 지역승수에 대한 인식 개선(정답률 : 57% => 60% => 62% => 63%)
     + Monty Hall 문제에 대한 인식 우수(정답률 90%) 
     + Wason selection 1에 대한 인식 양호(정답률 : 75%)
     + Wason selection 2에 대한 인식 양호(정답률 : 71%)
     
```{r, cross-tables}
load("../R/quiz1028v2.rda")
quiz1125.div <- data.frame(quiz1028.div[, 1:7], quiz1125[c("Q1", "Q2", "Q3", "Q4")])
pander(table(quiz1125.div$college, quiz1125.div$Q1))
pander(table(quiz1125.div$college, quiz1125.div$Q2))
pander(table(quiz1125.div$college, quiz1125.div$Q3))
pander(table(quiz1125.div$college, quiz1125.div$Q4))
```

```{r, mapply and xtabs}
pander(mapply(function(x) xtabs(~college + x, data=quiz1125.div), quiz1125.div[c("Q1", "Q2", "Q3", "Q4")], SIMPLIFY = FALSE))
```

```{r, mosaicplots, fig.width = 6, fig.height = 6}
mosaicplot(table("단과대학"=quiz1125.div$college, "Q1"=quiz1125.div$Q1), main="단과대학별 응답(Q1)")
mosaicplot(table("단과대학"=quiz1125.div$college, "Q2"=quiz1125.div$Q2), main="단과대학별 응답(Q2)")
mosaicplot(table("단과대학"=quiz1125.div$college, "Q3"=quiz1125.div$Q3), main="단과대학별 응답(Q3)")
mosaicplot(table("단과대학"=quiz1125.div$college, "Q4"=quiz1125.div$Q4), main="단과대학별 응답(Q4)")
```

```{r, mapply with mosaicplot, fig.width = 6, fig.height = 6}
mapply(function(x) mosaicplot(xtabs(~college + x, data=quiz1125.div), main=paste("단과대학별 응답(", dimnames(x)[[2]],")", sep=""), xlab = "단과대학", ylab = names(x)), quiz1125.div[c("Q1", "Q2", "Q3", "Q4")])
```
...
pander(table(q.ans.monty.hall))
pander(table(q.ans.wason1))
pander(table(q.ans.wason2))
```
...
```{r}
(marks <- mapply(function(x, y) factor(ifelse(x == y, "R", "W")), y = q.key.l, quiz1125[c("Q1", "Q2", "Q3", "Q4")]))
```
...
```{r}
v.names <- c("고성지역자활\nLM3파급효과", "Monty Hall\n 정답률", "Wason Selection 1 \n정답률", "Wason Selection 2\n 정답률")
```
...
```{r}
barplot(marks.tbl.2, col=c("cyan", "red"), names.arg = v.names)
title(main="11월 25일 퀴즈 결과", xlab="문항", ylab="인원")
```
...
library(reshape2)
str(marks)
dimnames(marks) <- list("ID" = quiz1125$id, "Items" = dimnames(marks)[[2]])
...
ls()
(ml.3 <- ml.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("11월 25일 퀴즈 결과"))
```
...
```{r}
(mtl.3 <- mtl.2 + theme.HCR.kr + xlab("문항") + ylab("인원") + ggtitle("11월 25일 퀴즈 결과"))
```
